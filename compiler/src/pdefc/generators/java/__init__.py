...
from pdefc.generators import upper_first
...
class JavaGenerator(generators.Generator):
    def __init__(self, out, namespace=None, **kwargs):
        self.out = out
        self.namespace = generators.Namespace(namespace)
        self.filters = JavaFilters(self.namespace)
        self.templates = generators.Templates(__file__, filters=self.filters)
    ...
    def _write(self, def0):
        ...
        generators.mkdir_p(package_path)
    ...
    def _render(self, def0):
        tname = TEMPLATE_NAMES.get(def0.type)
        if not tname:
            raise ValueError('Unsupported definition %r' % def0)

        template = self.templates.get(tname)
        return template.render(generated_by=GENERATED_BY, definition=def0, **def0.__dict__)
...
JAVA_NATIVE_REFS = {
    TypeEnum.BOOL:  JavaRef('Boolean',  'io.pdef.descriptors.Descriptors.bool', 'boolean', 'false'),
